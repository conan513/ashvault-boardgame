...
// Frakció kártya húzása
function drawFactionCard(faction) {
  if (!decksState || !decksState[faction] || decksState[faction].length === 0) {
    resetDecksState();
  }
  return decksState[faction].shift();
}

// Equipment kártya húzása
function drawEquipmentCard() {
  if (!decksState || !decksState["Equipment"] || decksState["Equipment"].length === 0) {
    resetDecksState();
  }
...
function drawEnemyCard() {
  if (!decksState || !decksState["Enemies"] || decksState["Enemies"].length === 0) {
    resetDecksState();
  }
